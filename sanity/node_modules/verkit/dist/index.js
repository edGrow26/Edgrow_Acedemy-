...
function formatFullVersion(version) {
	...
	return version.build.length > 0 ? `${comparable}+${version.build.join(".")}` : comparable;
}
...
function incrementPrerelease(version, identifier, identifierBase) {
	...
	if (version.prerelease.length === 0) version.prerelease = [base];
	else {
		let foundNumeric = false;
		for (let index = version.prerelease.length - 1; index >= 0; index--) if (typeof version.prerelease[index] === "number") {
			version.prerelease[index] = Number(version.prerelease[index]) + 1;
			foundNumeric = true;
			break;
		}
		if (!foundNumeric) {
			if (identifier === version.prerelease.join(".") && identifierBase === false) throw new Error("invalid increment argument: identifier already exists");
...
function incrementMutable(version, release, identifier, identifierBase) {
	switch (release) {
		...
		case "prepatch":
			...
			break;
		case "prerelease":
			if (version.prerelease.length === 0) incrementMutable(version, "patch", identifier, identifierBase);
			incrementPrerelease(version, identifier, identifierBase);
			break;
		case "release":
			if (version.prerelease.length === 0) throw new Error(`version ${formatFullVersion(version)} is not a prerelease`);
			version.prerelease = [];
			break;
		case "major":
			if (version.minor !== 0 || version.patch !== 0 || version.prerelease.length === 0) version.major++;
			version.minor = 0;
			version.patch = 0;
			version.prerelease = [];
			break;
		case "minor":
			if (version.patch !== 0 || version.prerelease.length === 0) version.minor++;
			version.patch = 0;
			version.prerelease = [];
			break;
		case "patch":
...
function parseRange(range, options = {}) {
	...
	let sets = raw.split("||").map((part) => parseSimpleRange(part.trim(), parsedOptions)).filter((set) => set.length > 0);
	if (sets.length === 0) throw new TypeError(`Range contains no valid comparator sets: ${raw}`);
	if (sets.length > 1) {
		const first = sets[0];
...
function testComparatorSet(set, version, options) {
	if (!set.every((comparator) => testParsedComparator(comparator, version))) return false;
	...
	return set.some((comparator) => {
		...
		return allowed !== null && allowed.prerelease.length > 0 && allowed.major === version.major && allowed.minor === version.minor && allowed.patch === version.patch;
	});
}
...
function isSatisfiable(comparators, options) {
	...
	while (current && remaining.length > 0) {
		if (!remaining.every((other) => parsedComparatorsIntersect(current, other, options))) return false;
		current = remaining.pop();
	}
	return true;
}
function parsedRangesIntersect(left, right, options = {}) {
	return left.sets.some((leftSet) => isSatisfiable(leftSet, options) && right.sets.some((rightSet) => isSatisfiable(rightSet, options) && leftSet.every((leftComparator) => rightSet.every((rightComparator) => parsedComparatorsIntersect(leftComparator, rightComparator, options)))));
}
//#endregion
//#region src/range.ts
...
function nextVersionAfter(version) {
	return parse(version.prerelease.length === 0 ? formatComparableVersion({
		major: version.major,
		minor: version.minor,
...
function samePrereleaseTuple(comparator, version) {
	const candidate = comparator.version;
	return candidate !== null && candidate.prerelease.length > 0 && candidate.major === version.major && candidate.minor === version.minor && candidate.patch === version.patch;
}
function simpleRangeSubset(rawSubset, rawSuperset, options) {
	...
	for (const version of equal.values()) {
		...
		return superset.every((comparator) => testParsedComparator(comparator, version));
	}
	let needsLowerPrerelease = lower && !options.includePrerelease && lower.version.prerelease.length > 0 ? lower.version : null;
	let needsUpperPrerelease = upper && !options.includePrerelease && upper.version.prerelease.length > 0 ? upper.version : null;
	if (needsUpperPrerelease?.prerelease.length === 1 && upper?.operator === "<" && needsUpperPrerelease.prerelease[0] === 0) needsUpperPrerelease = null;
	...
	for (const comparator of superset) {
		...
		if (lower) {
			if (needsLowerPrerelease && samePrereleaseTuple(comparator, needsLowerPrerelease)) needsLowerPrerelease = null;
		...
		if (upper) {
			if (needsUpperPrerelease && samePrereleaseTuple(comparator, needsUpperPrerelease)) needsUpperPrerelease = null;
			if (comparator.operator === "<" || comparator.operator === "<=") {
...
function difference(left, right) {
	...
	if (low.prerelease.length > 0 && !highHasPrerelease) {
...
//#endregion
export { INCREMENT_TYPES, SEMVER_SPEC_VERSION, TRUNCATION_TYPES, clean, coerce, comparatorsIntersect, compare, compareBuild, compareIdentifiers, compareIdentifiersReversed, compareMain, comparePrerelease, compareReversed, compareWithOperator, difference, findMaxSatisfying, findMinSatisfying, findMinimumForRange, getBuild, getMajor, getMinor, getPatch, getPrerelease, increment, isEqual, isGreater, isGreaterOrEqual, isGreaterThanRange, isLess, isLessOrEqual, isLessThanRange, isNotEqual, isOutsideRange, isRangeSubset, isValid, isValidRange, normalize, normalizeComparator, normalizeFull, normalizeRange, parse, parseRange, rangeToComparators, rangesIntersect, satisfies, satisfiesComparator, simplifyRange, sort, sortReversed, truncate, tryParse, tryParseRange };
