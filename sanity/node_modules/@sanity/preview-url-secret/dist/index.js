import { apiVersion, urlSearchParamPreviewSecret, urlSearchParamPreviewPerspective, urlSearchParamPreviewVariant, urlSearchParamPreviewPathname, urlSearchParamVercelProtectionBypass, urlSearchParamVercelSetBypassCookie, fetchSecretQuery, fetchSharedAccessSecretQuery, tag, isDev } from "./constants.js";
...
function parsePreviewUrl(unsafeUrl) {
  ...
  const studioPreviewPerspective = url.searchParams.get(urlSearchParamPreviewPerspective), studioPreviewVariant = url.searchParams.get(urlSearchParamPreviewVariant) || null;
  let redirectTo;
  ...
  if (unsafeRedirectTo) {
    ...
    studioPreviewPerspective && !redirectUrl.searchParams.has(urlSearchParamPreviewPerspective) && redirectUrl.searchParams.set(urlSearchParamPreviewPerspective, studioPreviewPerspective), studioPreviewVariant && !redirectUrl.searchParams.has(urlSearchParamPreviewVariant) && redirectUrl.searchParams.set(urlSearchParamPreviewVariant, studioPreviewVariant), url.searchParams.has(urlSearchParamVercelProtectionBypass) && (redirectUrl.searchParams.set(
...
async function validatePreviewUrl(_client, previewUrl, disableCacheNoStore = globalThis.navigator?.userAgent === "Cloudflare-Workers") {
  ...
  const { isValid, studioUrl } = await validateSecret(
    ...
    disableCacheNoStore
  ), redirectTo = isValid ? parsedPreviewUrl.redirectTo : void 0, studioPreviewPerspective = isValid ? parsedPreviewUrl.studioPreviewPerspective : void 0, studioPreviewVariant = isValid ? parsedPreviewUrl.studioPreviewVariant : void 0;
  let studioOrigin;
  if (isValid)
    try {
