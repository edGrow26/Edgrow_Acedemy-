import { fromPromise, setup, assign, sendParent, fromObservable, fromCallback, and, stateIn, enqueueActions, sendTo, raise } from "xstate";
import { logger } from "./_chunks-es/index.js";
import { getAuthState, resolveOrganization, logout, AuthStateType, getClient, createSanityInstance } from "@sanity/sdk";
import { os as os$1, defineStateTopics } from "./_chunks-es/bus.js";
...
const authStateLogic = fromObservable(
...
), organizationResolverLogic = fromPromise(
  async ({ input }) => {
    const organization = await resolveOrganization(input.instance, {
  ...
  actors: {
    ...
    organizationResolver: organizationResolverLogic,
  ...
  actions: {
    // Seed the (suspending) topic before any publish, or a state emit would be
    // misrouted as an event.
    seedTopics: () => {
      defineStateTopics(os$1, {
        "auth.token": void 0,
        "organizations.current": void 0,
        "users.current": void 0
      });
    },
  ...
  states: {
    ...
    [AuthStateType.LOGGED_IN]: {
      ...
      // The session is only established once its organization context resolved:
      // `authenticated` (and with it boot readiness) waits for the resolve, so
      // a failed resolve fails boot exactly like a failed login.
      initial: "resolving-organization",
      states: {
              ...
              actions: [
                {
                  ...
                  params: ({ event }) => ({ organization: event.output })
                }
              ]
            },
            onError: {
              target: `#auth.${AuthStateType.ERROR}`,
              actions: [
                {
                  type: "setError",
                  params: ({ event }) => ({
                    error: new Error(
                      "Failed to resolve the session's organization",
                      { cause: event.error }
                    )
                  })
                }
        ...
        ready: {
          tags: ["authenticated"],
          // Leaving `ready` unsubscribes the responder, so signed-out callers
          // get NO_RESPONDER instead of a token.
    ...
    [AuthStateType.LOGGED_OUT]: {
      entry: [
        { type: "publishCurrentUser" },
        { type: "publishToken" },
        { type: "publishOrganization", params: { organization: null } }
      ]
    },
    [AuthStateType.ERROR]: {
      tags: ["error"],
...
const servicesLogic = setup({
  ...
  actions: {
    // Seeding with the synchronously-resolved value doubles as the initial
    // publish — reads never suspend.
    seedColorScheme: ({ context }) => {
      defineStateTopics(os$1, {
        "preferences.color-scheme": context.colorScheme
      });
    },
