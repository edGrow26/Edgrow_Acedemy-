...
export class ImportDatasetCommand extends SanityCommand {
    ...
    async run() {
        ...
        if (targetDatasetArg && !datasetFlag) {
            this.output.warn('Positional dataset argument is deprecated. Use the --dataset flag instead: --dataset <name>');
        }
        ...
        try {
            ...
            return err.name === 'ReplacementCharError' ? this.output.error(`Import failed due to unicode replacement characters in the data.\n${err.message}\n\nIf you are certain you want to proceed with the import despite potentially corrupt data, re-run the import with the \`--allow-replacement-characters\` flag set.`, {
                exit: exitCodes.RUNTIME_ERROR
            }) : this.output.error(err.stack || err.message, {
                exit: exitCodes.RUNTIME_ERROR
            });
        }
    }
    ...
    printWarnings(warnings) {
        ...
        this.output.warn(`Failed to import the following ${assetFails.length > 1 ? 'assets' : 'asset'}:`);
