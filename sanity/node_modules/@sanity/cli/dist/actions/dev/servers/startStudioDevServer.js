...
export async function startStudioDevServer(options) {
    ...
    try {
        ...
        if (loadInDashboard) {
            ...
            spin.succeed();
            loggerInfo(`${appType} ` + `using ${styleText('cyan', `vite@${viteVersion}`)} ` + `ready in ${styleText('cyan', `${Math.ceil(startupDuration)}ms`)}` + (announceUrl ? ` and running at ${styleText('cyan', url)}` : ''));
        }
        return {
            close,
            server,
